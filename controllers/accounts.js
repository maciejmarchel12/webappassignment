...
const accounts = {
 ...
 //register function to render the registration page for adding a new user
  register(request, response) {
    ...
    response.cookie('aircraft', user.email);
  ...
  authenticate(request, response) {
    ...
    if (user && user.password==request.body.password && user.email==request.body.email) {
      response.cookie('aircraft', user.email);
      logger.info('logging in' + user.email);
      response.redirect('/start');
    } else {
 ...
 //utility function getCurrentUser to check who is currently logged in
  getCurrentUser (request) {
    const userEmail = request.cookies.aircraft;
